...
Commander.prototype = {
	...
	_promotion: function(){
		this.ready = true;
		console.log(LOGKEY + ' promoted to', this.type);
		return this;
	},
	...
	_buildIndex : function(){
		...
		console.log(LOGKEY + 'applications:', this.applications.length);
	},
	getCfg: function(){
		return this.cfg;
	},
	...
	run: function(){
		...
		//while(hasTests || hasApplications){
			console.log(LOGKEY + 'has Applications || hasTests');
			...
			console.log(LOGKEY + 'no remaining tests, shutting down');
			if(this.finished){
				this.finished(false)
			}else{
				process.exit(0);				
	...
	stop: function(){
		...
		if(this.child){
			console.log(LOGKEY + 'stopping commander child');
			this.child.kill();
			setTimeout(function(){
				deferred.resolve('stop');
			}, 500);
		}else{
			console.log(LOGKEY + 'Cant stop. No child process running.');
		}
	...
	next: function(done){
		...
		console.log(LOGKEY + 'commander.next', this.currentTest);
		...
		if(this.currentTest + 1 <= this.getCurrentApplicationTestLength()){
			...
			if(this.currentApplication + 1 < this.applications.length){
				...
				console.log(LOGKEY + 'no remaining commands and applications, calling done');
	...
	_exec: function(done){
		...
		this.child.on('close', function(code){
			console.log(LOGKEY + 'child process terminated code:' + code);
			done()
		});

		this.child.stdout.on('data', function (data) {
		  console.log(LOGKEY + 'stdout: ' + data);
		});

		this.child.stderr.on('data', function (data) {
		  console.log(LOGKEY + 'stderr: ' + data);
		});
